...
<body>
<div class="wrap">
<div class="head text-center" onclick="memberInfo('${memberBaseDto.name}')">
  <span class="">${memberBaseDto.name}</span>
  <div class="yuangong-font">
    <c:choose>
	       ...
	       <div class="lunpai-project">
	            <div class="lunpai-project-part">
	                <div class="lunpai-project-title">
	                    ...
	                    <!-- <span class="waiting-num">还有<span class="num">3</span>位客人在等待</span> -->
                            ...
                            <c:forEach items="${projectObj.shiftMahjongDtoList}" var="shiftMahjongDto" varStatus="shiftStatus">
                                <c:choose>
                                    <c:when test="${shiftStatus.index == 0}">
                                        <li class="item active" onclick="switchoverDIV(this, ${listStatus.index}, ${shiftStatus.index})">${shiftMahjongDto.shiftMahjongName}</li>
                                    </c:when>
                                    <c:otherwise>
                                        <li class="item" onclick="switchoverDIV(this, ${listStatus.index}, ${shiftStatus.index})">${shiftMahjongDto.shiftMahjongName}</li>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                        </ul>
                    ...
                    <hr>
	                 <c:forEach items="${projectObj.shiftMahjongDtoList}" var="shiftMahjongDto" varStatus="shiftStatus">
	                   <div <c:if test="${shiftStatus.index == 0}">class="lunpai-people"</c:if> <c:if test="${shiftStatus.index != 0}">class="lunpai-people hide"</c:if> name= "${shiftStatus.index}">
	                        <div class="swiper-container-daohang lunpai lunpaiHead${listStatus.index}${shiftStatus.index }" size="${fn:length(shiftMahjongDto.shiftMahjongEmployeeList)}">
	                            <div class="swiper-wrapper lunpaiBody${listStatus.index}${shiftStatus.index }">
	                               <c:forEach items="${shiftMahjongDto.shiftMahjongEmployeeList}" var="shiftMahjongEmployee" varStatus="status">
	                                  <div class="swiper-slide score-shop-li line-height-center s-modal-control" data-target="#yg-rotating" onclick="chooseEmployee(this)">
	                                        <div class="lunpai-img" name= "${shiftMahjongEmployee.shiftMahjongEmployeeId}" projectNum = "${listStatus.index}" orderNum = "${shiftStatus.index}" projectId = "${projectObj.projectId}" employeeId = "${shiftMahjongEmployee.employeesId}" isType = "0">
...
/* 	    var clientWidth = $(window).width() - 16;
    	...
    	for (var i = 0; i < totilSize; i++) {
            var s = new Swiper(".lunpaiHead" + i + 0, {
                slidesPerView: 'auto',
